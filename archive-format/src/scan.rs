...
use crate::{ArchiveStore, SessionArchiveManifest};

/// One archived session, flattened to the (user, manifest) pair every reader
/// works from.
#[derive(Debug, Clone)]
pub struct FlatRow {
    pub user_id: Uuid,
...
/// and unreadable per-user listings are reported through `warn` and skipped;
/// only a failure to list users at all is fatal (nothing can be done without
/// it). `warn` lets each caller keep its own sink (stderr for the CLI,
/// `tracing` for the backend).
pub fn collect_rows(
    store: &ArchiveStore,
    warn: &mut dyn FnMut(String),
) -> std::io::Result<Vec<FlatRow>> {
    ...
    for user_id in users {
        let sessions = match store.list_sessions(user_id) {
            ...
            Err(e) => {
                warn(format!(
                    "skipping user {user_id}: cannot list sessions: {e}"
                ));
                continue;
            }
        };
        for session_id in sessions {
            match store.get_session_manifest(user_id, session_id) {
                ...
                Err(e) => {
                    warn(format!(
                        "skipping corrupt manifest for session {session_id} (user {user_id}): {e}"
                    ));
                }
            }
        }
...
/// Parse a `from`/`to` bound: RFC3339 (any offset, normalized to UTC) or a
/// bare `YYYY-MM-DD`. A bare date becomes start-of-day for `from` and
/// end-of-day for `to` (`end_of_day = true`) so a single date is an inclusive
/// full-day bound.
pub fn parse_date_arg(input: &str, end_of_day: bool) -> Result<NaiveDateTime, String> {
    ...
    if let Ok(date) = NaiveDate::parse_from_str(input, "%Y-%m-%d") {
        ...
        return time.ok_or_else(|| format!("invalid date `{input}`"));
    }
    Err(format!(
