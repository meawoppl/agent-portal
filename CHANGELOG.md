...
- **Retire the `CodexFrameCaptureLayer` tracing workaround from 2.5.30.** The layer was meant to recover the raw codex frame on typed-decode failures by snooping codex-codes' `[CLIENT] Received: …` DEBUG events out of band, but in practice the capture didn't fire — likely a callsite-cache / per-layer-filter interaction in tracing-subscriber 0.3 that the unit test didn't exercise. Rather than chase that down, [SDK issue #128](https://github.com/meawoppl/rust-code-agent-sdks/issues/128) is being implemented: codex-codes will gain a `ParseError` carrying `raw_line` / `raw_json` / `error_message`, mirroring claude-codes' existing pattern. Once that ships, agent-portal will match on `codex_codes::Error::Deserialization(parse_err)` and render `raw_json` directly — the same way the claude path has always handled unparsable frames. Pulled out: the layer module, its install in `launcher/src/main.rs` and `proxy/src/main.rs`, the `tracing-subscriber` dep from `claude-session-lib`, and the per-layer EnvFilter rewiring. The 2.5.26 turn-interrupt recovery from #703 is unchanged — the user still gets a `turn.failed` portal message and an unwedged session; we just don't try to attach the raw frame in the interim.
