...
- **Dedupe codex item-lifecycle events in `render_codex_group` (closes #776).** A single codex bash command lands on the wire as `item.started { id: "c1", status: "running", … }` immediately followed (after exec) by `item.completed { id: "c1", status: "completed", aggregated_output, exit_code, … }` — and any number of `item.updated` events in between when output streams. Before this fix, each landed as a separate transcript card, so a `cargo test` invocation showed up as two near-identical "Codex / commandExecution" cards (running + completed) side-by-side, and a long-streaming MCP call could show many. `render_codex_group` now runs a `codex_renderer::dedupe_lifecycle` pass first: parses each raw JSON into the typed `CodexEvent` (per #767), extracts a `(item_type_tag, item_id)` key off the `ItemStarted | ItemUpdated | ItemCompleted` arm's `CodexItem`, and collapses runs of the same key to only the latest occurrence (which is `ItemCompleted` when the item finished, the latest in-flight `ItemUpdated` when it's mid-stream, or the lone `ItemStarted` for a hung command). The surviving message sits at the position of its latest occurrence so the transcript reflects when the item *finished*, not when it started. Non-lifecycle events (TurnCompleted, streaming-delta variants), lifecycle events on `CodexItem::Unknown` (no id), and malformed-JSON entries pass through unchanged so we never silently drop a message. Dedup is keyed on the type tag too — a `command_execution` with `id="a"` and a `file_change` with `id="a"` correctly stay separate. Live-stream UX is also better: the in-progress card is visible until the completion arrives, then naturally replaced on the next re-render. Old session history reloaded from the DB is cleaned up retroactively for free. Six new unit tests in `codex_renderer.rs::tests` cover the motivating bash regression, multi-update streaming runs, independent items not collapsing, non-lifecycle passthrough, items-without-id passthrough, and malformed-JSON passthrough.
