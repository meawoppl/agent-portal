...
- **Drop the duplicate "Codex" badge from `render_agent_message`.** Follow-up to PR #778 (PR 3/4 of #758: User + Codex grouping). `render_codex_group` wraps a consecutive run of codex events in an outer `.codex-group` card whose header carries a `Codex` `message-type-badge` — but `render_agent_message` (which renders `CodexItem::AgentMessage` inside that group) was also rendering its own `<span class="message-type-badge assistant">Codex</span>` header, so every assistant text card inside a group showed "Codex" twice: once at the group top, once on the card. The fix drops the per-item header div entirely; the surrounding group already provides the "Codex" framing, and other `CodexItem` renderers (`render_reasoning`, `render_command_execution`, `render_file_change`, `render_mcp_tool_call`, `render_web_search`, `render_todo_list`) never had a "Codex" badge in the first place (they have purpose-specific labels like "Bash" / "reasoning" / "File Changes"), so this brings agent messages in line with the rest. The per-item `.claude-message.assistant-message` card wrapper stays (preserves the existing nested-card styling for the assistant text), only the redundant header row is gone. Bytewise no change to what the renderer emits when the agent message text is empty (it still returns `html! {}`).
