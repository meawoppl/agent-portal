...
This is operator tooling over raw archive storage. The **authenticated web
history browser** lives in the portal app itself: the backend's
`/api/history/*` endpoints (visibility-scoped per user: own sessions, sessions
shared via `session_members`, everything for admins) and the
`frontend/src/pages/history/` UI at `/history`.
